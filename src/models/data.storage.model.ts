...
import { DataSecureAbstract } from "./data.message.abstract";
 ...
 *  Note: the deactivation date is the "updated" timestamp.
 */
export interface DataStorageBase { // old StorageBase
...
export interface TxCompositionUnencrypted extends
    TxCompositionBase
{
    content?:   SecureDataAbstract;   // decrypted payload, it can be encrypted as the `jwe` element before being stored.
...
export interface IndexProtected {
    attributes: IndexedAttributeProtected[]; // HMAC attribute name and value
    hmac:       IDTypePair;
    sequence:   number;
}
...
export interface SecureStorageData extends // note: old StorageBase
    DataStorageBase
{  
    indexed?:       IndexProtected;              // safe indexed attributes (HMAC).
